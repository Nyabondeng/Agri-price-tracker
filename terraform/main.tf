...
# ==================================================
# App Service Infrastructure (Public Deployment)
# ==================================================
# This runs independently from Ansible + VM
# Both paths can coexist without affecting each other

# Managed Identity for App Service to pull from ACR
resource "azurerm_user_assigned_identity" "app_service" {
  name                = "id-appservice-${random_string.suffix.result}"
  resource_group_name = azurerm_resource_group.main.name
  location            = azurerm_resource_group.main.location
  tags                = var.tags
}
...
resource "azurerm_role_assignment" "appservice_acr" {
...
resource "azurerm_service_plan" "main" {
  ...
  location            = azurerm_resource_group.main.location
  os_type             = "Linux"
  sku_name            = "S1"  # Standard tier for auto-scaling and custom domains
  tags                = var.tags
}
...
resource "azurerm_linux_web_app" "main" {
  ...
  app_settings = {
    ...
    WEBSITES_PORT                       = "3000"  # Port where Node.js app listens
    DOCKER_REGISTRY_SERVER_URL          = "https://${azurerm_container_registry.main.login_server}"
    DOCKER_ENABLE_CI                    = "true"
    DOCKER_CUSTOM_IMAGE_NAME            = "${azurerm_container_registry.main.login_server}/agri-price-tracker:latest"
    DOCKER_REGISTRY_SERVER_USERNAME     = "24b5e19c-c0ac-4b00-b31b-dd77313e2a54"  # Managed identity client ID (placeholder, will use identity)
  }
  ...
  site_config {
    application_stack {
      docker_image_name   = "${azurerm_container_registry.main.login_server}/agri-price-tracker:latest"
      docker_registry_url = "https://${azurerm_container_registry.main.login_server}"
    }
    
    use_32_bit_worker_process = false
    http2_enabled             = true
    container_wrap_startup_command = ""
  }

  depends_on = [azurerm_role_assignment.appservice_acr]
}
